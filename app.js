...
function getGift(age, gen) {
  // console.log(`gift age ${age}`);
  // console.log(`gift gender ${gen}`);
  ...
  if (gen === "male") {
    preSent = mPres[ranGe][rNum(mPres[ranGe].length)];
    // return preSent;
    console.log(preSent);
  }

  if (gen === "female" || gen === "unconfirmed") {
    preSent = fPres[ranGe][rNum(fPres[ranGe].length)];
    // return preSent;
    console.log(preSent);
  }
...
function formTwo() {
  ...
  createElem("p", "p2", "", "", "A few more steps are required....", "", "", "");
  ...
  dataForm.addEventListener("submit", (e) => {
    ...
    // todo: remove a list of elements?
...
function funcThree() {
  ...
  giftBtn.addEventListener("click", () => {
    // console.log(`age ${userData.howOld}`);
    // console.log(`gender ${userData.userGen}`);
...
if (str) { // stats exist in local storage
  ...
  // todo: get telephone number after form submission, add number to local storage
